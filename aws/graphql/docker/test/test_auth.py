...
import pytest as pt
from entity.user import UserToken, login_for_access_token
...
def test_login(mock_dao, user_request, user_response):
    ...
    user_token: UserToken = login_for_access_token(user_request.get('user_name'), user_request.get('password'))
    assert user_token
    user = get_current_user(user_token.token)
