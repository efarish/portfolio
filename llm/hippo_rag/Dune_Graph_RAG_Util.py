...
def word_count(txt: str)->int:
    """
    Return of GPT2 tokens found in a string.
    """
...
def execute_query(query, parameters=None):
    ...
    Parameters:
    query (str): Cypher MATCH statement.
    parameters (dict): key/value pairs query.
...
def get_entity_llm():
    """
    Create a LangChain pipeline to perform named entity recognition (NER) on text documents.
    """
    ...
    llm = ChatOpenAI(temperature=0, model_name=LLM_MODEL) #gpt-3.5-turbo-1106 gpt-4o
    summary_chain = summary_prompt | llm | parser

    return summary_chain

def initialize_entities(entities: list, src_text):
    """
    ...
    for entity in entities:
        embed = embedder.encode(entity.name + "|" + entity.type)
        entity.embedding = embed
        entity.entity_alias = [] 
        entity.src_reference = set()
        entity.src_reference.add(src_text)
    #return _entities
...
def cypher_entity_create(e: AnEntity)->str:
    ...
    """
    stmt = '\n' + f"CREATE (n {{name: '{e.name.replace("'","\\'")}', \
...
def cypyer_entity_alias(e_alias: AnEntity, e: AnEntity)->str:
    """
    Create an also_known_as relationship between an entity and its alias names.
    """
...
def add_embeddings_to_nodes(entities: List[AnEntity]) -> None:
    """
    Create a vector embedding use the HuggingFace SentenceTransformer model. This creates a 364 dimension 
      embedding vector.
    """
    ...
    for entity in entities:
        entity.embedding = embedder.encode(entity.name)
        entities_with_embeddings.append( {'name': entity.name, 'embedding':  entity.embedding} )
    ...
    with driver.session(database="neo4j") as session:
        ...
        MATCH (m {name: entity.name})
        CALL db.create.setNodeVectorProperty(m, 'embedding', entity.embedding)
        ''', entities=entities_with_embeddings, database_="neo4j")
    driver.close()  

def do_index_similarity_search_using_index(query_prompt, score=0.80):
    """
    Search a Neo4j vector index for entity names that have a similarity score
      above the "score" threshold.
    """
    embedder = SentenceTransformer('paraphrase-MiniLM-L6-v2')
    query_embedding = embedder.encode(query_prompt)
    driver = GraphDatabase.driver(neo4j_url, auth=(neo4j_user, neo4j_password))
    related_entities, _, _ = driver.execute_query('''
    CALL db.index.vector.queryNodes('entityIdx', 5, $queryEmbedding)
    YIELD node, score
    RETURN node.name AS name, '' as type, '' AS description, score
    ''', queryEmbedding=query_embedding,
    database_="neo4j")

    related_entities = [AnEntity(**dict(d)) for d in related_entities if d['score'] > score]
    
    related_entities.sort(key=lambda x: -x.score)
...
def extract_entities_from_query(query: str) -> List[str]:
    """
    Extract entity names form text string use LLM.
    """
    llm=ChatOpenAI(temperature=0, model_name=LLM_MODEL) 
    ...
    class Entities(BaseModel):
        ...
        names: List[str] = Field(
            ...,
            description="All the person, organization, or locations that "
            "appear in the text",
        )
...
def get_PPR_Scores(name: str)->list:
  stmt = f"""
  MATCH (e1 {{name: '{name}'}})
  ...
  YIELD nodeId, score
  RETURN nodeId AS id, gds.util.asNode(nodeId).name AS name, score
  ORDER BY id
  """
  driver = GraphDatabase.driver(neo4j_url, auth=(neo4j_user, neo4j_password))
  ppr_scores, _, _ = driver.execute_query(stmt, database_="neo4j")

  ppr_scores = np.array([d['score'] for d in ppr_scores])

  return ppr_scores


def get_relationships(split: int, src_splits):
    relationship_llm = get_relationship_llm()
    stmt = f""" 
    ...
    for r in relationships:
        ...
        if stmt is not None:
            try:
                ...
                cypher_failures.append(stmt)
    if len(parse_failures):
        print(f'Parse errors occurred:\n{parse_failures}') 
    if len(cypher_failures):
        print(f'Cypher errors occurred:\n{cyper_failures}')
      

def get_RAG_chain():

    template = """Answer the question based only on the these documents:
    
    {context}
    
    Question: {question}

    Directions: Use natural language and be concise.
